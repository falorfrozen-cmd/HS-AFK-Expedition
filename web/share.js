...
window.AfkShare=(()=>{
  ...
  async function draw(summary,canvas){
    ...
    best.forEach((b,i)=>{const bx=44+i*140,by=380,c=COLORS[b.group]||COLORS[b.rarity]||COLORS.Other;
      ...
      ctx.fillStyle=c;ctx.font='16px Georgia, serif';const [one,two]=wrap2(ctx,b.name,132);ctx.fillText(one,bx,by+130);if(two)ctx.fillText(two,bx,by+150);});
    const notes=[];
    ...
    ctx.fillStyle='#ffd166';ctx.font='700 22px Georgia, serif';ctx.fillText(fit(ctx,notes.join('   ·   '),1110),44,540);
    ctx.fillStyle='#8f7a57';ctx.font='17px Georgia, serif';
    ctx.fillText(fit(ctx,`Every item was generated by the game's own drop calls · offline · AFK FARM v${summary.version||''}`,1110),44,582);
    return canvas;
  }
