...
function modifiers(){
 ...
 ${disclosure('modifier-help','How these modifiers apply','<p class="muted">Magic Find uses the game’s own calculation, not a guaranteed rarity chance. The final value includes native rounding and is confirmed on claim.</p><p class="settings-note">With a compatible ForgePact plugin, reward bonuses are isolated during AFK delivery so multipliers do not stack. Older ForgePact plugins need an update. Profiles recorded before independent rewards need a new calibration.</p>','Magic Find & ForgePact compatibility')}`;
}
