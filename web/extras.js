...
function notificationCard(){
  ...
  return `<section class="card extra-section"><span class="eyebrow">NOTIFICATIONS</span><h2>Know when to claim</h2><label class="notify-toggle"><input type="checkbox" id="ready-notification" ${on?'checked':''}> Windows notification when an expedition is ready, also while AFK FARM and the game are closed</label><p class="settings-note">A one-time Windows Task Scheduler task named AFK FARM\\Expedition ready shows it at the end time and then removes itself. Claiming or cancelling removes it too.</p>${status}</section>`;
}
...
function regionCard(){
  ...
  return `<section class="card extra-section"><span class="eyebrow">REGION COMPARISON</span><h2>Where ${esc(current()?.name||'your hero')} farms best</h2><div class="table-wrap"><table class="table region-table"><thead><tr><th>Region</th>${cols.map(([k,label])=>`<th><button class="sort-header ${regionSort===k?'active':''}" data-region-sort="${k}" aria-pressed="${regionSort===k}">${label}</button></th>`).join('')}<th>Based on</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${esc(r.name)}</strong></td>${cols.map(([k,,digits])=>{const v=value(r,k);return `<td class="${v!==null&&v===best[k]?'best':''}">${v===null?'—':num(v,digits)}</td>`;}).join('')}<td><small>${esc(basis(r))}</small></td><td>${r.usable?`<button class="inline-link" data-use-profile="${esc(r.profile)}">Plan here</button>`:'<small>Needs recalibration</small>'}</td></tr>`).join('')}</tbody></table></div><p class="settings-note">XP per hour comes from your calibration in that region, before reward settings. Gold and item columns come from expeditions you delivered there: gold at ×1, items at the Magic Find you used. Short expeditions give rough numbers. Click a column to sort.</p></section>`;
}
