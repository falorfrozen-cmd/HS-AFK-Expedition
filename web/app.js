...
function deliveryActive(){
  ...
  return !!data?.armed&&data.job?.action==='claim'&&data.job.state==='running'
...
function deliveryCard(){
  ...
  const pr=data.progress;
  return `<section class="card extra-section"><span class="eyebrow">CURRENT REWARD DELIVERY</span><h2>${deliveryTitle()}</h2><p class="muted">${esc(deliveryMessage())}</p><div class="progress-track"><i id="delivery-progress" style="width:${Number(pr.percent)||0}%"></i></div><div class="estimate"><div class="label-row"><span>Reward calls</span><strong id="delivery-calls">${num(pr.calls_done||0)} / ${num(pr.calls_total||0)}</strong></div><div class="label-row"><span>Generated items</span><strong id="delivery-items">${num(pr.items||0)}</strong></div><div class="label-row"><span>Hidden by your loot filter</span><strong id="delivery-filtered">${num(pr.items_filtered||0)}</strong></div></div><p class="settings-note">Counts are provisional. The game save and Vault transfer are confirmed separately after generation finishes.</p></section>`;
}
...
function jobBanner(){
  ...
  const pages={start:'explore',plan:'explore',claim:'explore',cancel:'explore',recover:'explore',capture_start:'measure',capture_stop:'measure',validate_start:'measure',restart_region:'measure',ingest:'loot',install:'settings',configure:'settings',portrait:'settings',save_modifiers:'modifiers'};
...
function updateLive(){
  ...
  if(data.armed){
    ...
    const note=document.getElementById('expedition-note');if(note)note.textContent=deliveryNeedsReview()?'Partial delivery is on hold. Review the save and reward records before any further delivery.':deliveryActive()?deliveryTitle()+'. '+deliveryMessage():remaining?'remaining · '+num(credit/3600,2)+' hours accrued':'Expedition time is complete. Return to the recorded hero and region to generate rewards.';
  ...
  if(deliveryActive()){
    const pr=data.progress;
    for(const [id,value] of [['delivery-calls',num(pr.calls_done||0)+' / '+num(pr.calls_total||0)],['delivery-items',num(pr.items||0)],['delivery-filtered',num(pr.items_filtered||0)]]){
...
async function poll(){
  ...
  pollPromise=(async()=>{
    try{
      ...
      const c=data.calibration;
      const next=JSON.stringify([view,selected,room,world,data.characters,data.profiles.map(p=>[p.id,p.usable,p.built_at,p.problems]),data.armed,data.plan?.character,data.rewards.map(r=>[r.id,r.stages]),data.job?.state,data.job?.id,data.job?.error,data.live?.plugin,data.live?.room,data.live?.character,data.live?.farm_context?.hash,data.live?.capture_on,data.live?.replay_running,data.game_running,data.editor,data.installation,c?.session,c?.character,c?.room,c?.running,c?.invalid,c?.outcome?.status,c?.outcome?.profile_id,c?.save_error,data.progress.state,data.progress.pause,data.progress.reconciliation_required,data.recovery?.status,data.validations,data.portraits,data.reward_modifiers,data.claim_context_matches]);
      const editing=document.activeElement?.matches('#game-folder,#duration,#loot-search,#validation-profile,[data-reward]');
