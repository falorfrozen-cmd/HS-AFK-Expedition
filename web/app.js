...
function problemBox(){
  ...
  const r=data.recovery,at=`${num(r.calls_done||0)} of ${num(r.calls_total||0)} reward calls`;
  if(r.continuable)return `<div class="error-box">The previous claim stopped with an uncertain outcome at ${at}, without a confirmed save. Its item records end exactly at that position, so it can continue from there, or you can close it as a partial delivery.</div>`;
  const why=(r.continue_blockers||[]).length?`<br><small>It cannot continue from there: ${esc(r.continue_blockers.join(' '))}</small>`:'';
  return `<div class="error-box">The previous claim stopped with an uncertain outcome at ${at}. It cannot continue automatically, which prevents duplicate rewards. Close it as a partial delivery to keep what was delivered and start a new expedition.${why}</div>`;
}
function settlePartialButton(){
  const r=data.recovery,left=Math.max(0,(r.calls_total||0)-(r.calls_done||0)),off=busy()?'disabled':'';
  const report=`<a href="/api/recovery-report?id=${encodeURIComponent(r.id||'')}" target="_blank" rel="noopener">Recovery report</a>`;
  if(r.continuable)return `<button class="primary" data-accept-position ${off}>${svg('chest')} Continue from recorded position</button><div class="button-note">Delivers the remaining ${num(left)} reward calls when you claim again; delivered calls are not repeated. Whether the game saved XP and gold up to that point is not confirmed.</div><button class="quiet" data-settle-partial ${off}>Close as partial delivery</button><div class="button-note">Keeps what was delivered and gives up the rest. ${report}</div>`;
  return `<button class="primary" data-settle-partial ${off}>${svg('chest')} Close as partial delivery</button><div class="button-note">Keeps the rewards already delivered and gives up the remaining ${num(left)} reward calls. Nothing is generated again. ${report}</div>`;
}
function pausedBox(pr){
  if(pr.resume_accepted)return `<div class="availability">${svg('info')}<div><strong>Recorded position accepted</strong><p>${num(pr.calls_done||0)} of ${num(pr.calls_total||0)} reward calls were delivered before the interruption; whether the game saved their XP and gold is not confirmed. Continue with the same hero in the same region; delivered calls are not repeated.</p><button class="inline-link" data-settle-partial ${busy()?'disabled':''}>Close as partial delivery instead</button></div></div>`;
  return `<div class="availability good">${svg('check')}<div><strong>Delivery paused safely</strong><p>${num(pr.calls_done||0)} of ${num(pr.calls_total||0)} reward calls are delivered and saved. Continue with the same hero in the same region; nothing is delivered twice.</p></div></div>`;
}
...
// Gear, talents, levels or combat settings that changed since the calibration
// never block a claim; the calibrated pace still sets the rewards.
function loadoutNote(){
  const plan=data?.plan,live=data?.live;
  if(!plan||!live||!sameHero(plan.character,live.character)||data.claim_context_matches!==false||data.progress?.resumable)return '';
  return '<p class="settings-note">Your gear, talents, levels or combat settings changed since the calibration. That does not block the claim: the calibrated pace still sets the rewards.</p>';
}
function claimReadiness(){
  ...
  if(plan.game_build!==live.game_build)return 'This expedition was calibrated on another game version. It cannot be delivered on this one.';
...
// Disclosure choices survive polling, actions and page navigation.
const disclosureState=new Map();
function disclosure(id,title,body,meta='',open=false){
  const expanded=disclosureState.has(id)?disclosureState.get(id):open;
  return `<details class="disclosure" data-disclosure="${id}" ${expanded?'open':''}><summary id="${id}-summary"><span>${title}</span>${meta?`<small>${meta}</small>`:''}${svg('arrow')}</summary><div class="disclosure-body">${body}</div></details>`;
}
document.addEventListener('toggle',event=>{
  const el=event.target;if(el.matches?.('details[data-disclosure]')&&el.isConnected)disclosureState.set(el.dataset.disclosure,el.open);
},true);
function switchView(next){view=next;signature='';render();document.querySelector('#page-title')?.focus?.({preventScroll:true});window.scrollTo?.({top:0});}
function heading(eyebrow,title,description,button=''){
  return `<div class="page-heading"><div><span class="eyebrow">${eyebrow}</span><h1 id="page-title" tabindex="-1">${title}</h1><p>${description}</p></div>${button}</div>`;
}
...
function explore(){
  ...
  return heading('EXPEDITIONS',deliveryNeedsReview()?'Reward delivery needs review':deliveryActive()?'Generating your rewards':armed?'The adventure continues':'Where to next?',deliveryNeedsReview()?'Partial reward records are preserved. Delivery will not restart automatically.':deliveryActive()?'Keep the expedition hero in its recorded region until reward delivery finishes.':armed?'Play any hero or close the game. Your expedition keeps its own timer.':'Choose a region. Set your time. Let your hero’s measured pace do the rest.',gameLaunchButton())+jobBanner()+`
  <div class="explore-grid"><section class="atlas" aria-label="Interactive Hero Siege world map">
    ...
    <div class="atlas-top"><div><span class="eyebrow">WORLD MAP</span><div class="atlas-title">${world==='new'?'THE NEW WORLD':'TARETHIEL'}</div></div><div class="world-switch" aria-label="World selection"><button id="world-old" data-world="old" aria-pressed="${world==='old'}" class="${world==='old'?'active':''}">Tarethiel</button><button id="world-new" data-world="new" aria-pressed="${world==='new'}" class="${world==='new'?'active':''}">New World</button></div></div>
    ...
    <div class="region-tag">${svg('pin')} ACT ${esc(z.act)} · ${z.world==='new'?'NEW WORLD':'TARETHIEL'}</div><h2>${esc(z.name)}</h2><p class="region-description">${armed?'This expedition uses the profile saved when it started.':p?'Ready to explore at your measured pace.':'Calibrate your character’s farming pace in this region.'}</p>
    ...
    ${armed?expeditionPanel():`<div class="divider"></div><div class="label-row"><span>EXPEDITION DURATION</span><strong id="hours-label">${num(hours,2)} <small>h</small></strong></div><input id="duration" aria-label="Expedition duration (hours)" type="range" min="0.25" max="8" step="0.25" value="${hours}"><div class="range-labels"><span>15 min</span><span>8 h</span></div><div class="presets">${[1,2,4,8].map(h=>`<button id="duration-${h}" data-hours="${h}" aria-pressed="${hours===h}" class="${hours===h?'active':''}">${h} h</button>`).join('')}</div><div class="estimate"><div class="label-row"><span>Measured pace</span><strong>${p?num(p.kills_per_min,1)+' <small>kills/min</small>':'—'}</strong></div><div class="label-row"><span>Estimated kills</span><strong id="estimated-kills">${p?num(p.kills_per_min*hours*60):'—'}</strong></div></div><p class="modifier-summary">${rewardsSummary()} <button class="inline-link" data-view="modifiers">Edit modifiers</button></p><div class="availability ${p?'good':''}">${svg(p?'check':'info')}<div><strong>${p?(p.quality?.status==='steady'?'Steady measured pace':p.quality?.status==='variable'?'Pace varies across this sample':'Short measured sample'):esc(problem.title)}</strong><p>${p?`${num(p.basis_seconds/60,1)} min recorded · ${num(p.coverage*100,1)}% capture coverage. Saved profile ready.`:esc(problem.message)}</p></div></div>${p&&loadoutChanged(p)?loadoutNotice(p):''}<button class="primary" data-action="${p?'start':'measure'}" ${busy()?'disabled':''}>${svg(p?'play':'timer')} ${p?'Start expedition':'Calibrate farming pace'} ${svg('arrow')}</button><div class="button-note">${p?'Play another hero or close the game while time accrues.':'Measure in game once, then start offline expeditions.'}</div>${repeatCard()}`}
  </aside></div>${disclosure('expedition-help','How expeditions work',`<ol class="steps"><li><span class="step-number">01</span><div><strong>Measure your pace</strong><p>Record your normal route in a regular Act region.</p></div></li><li><span class="step-number">02</span><div><strong>Send your hero out</strong><p>Start from a saved profile. Play another hero or close the game.</p></div></li><li><span class="step-number">03</span><div><strong>Claim your rewards</strong><p>Return with the recorded hero to the same region. The game generates your loot.</p></div></li></ol>`,'A quick guide')}`;
}
...
function measurement(){
  ...
  return heading('CALIBRATION','Find your farming pace','Record a normal route. Save it once. Reuse it for offline expeditions.')+jobBanner()+`
  <div class="content-grid calibration-grid"><section class="card"><span class="eyebrow">${recording?'RECORDING IN PROGRESS':'CALIBRATION'}</span><div class="spacer"></div>
  ...
  <div class="notice">Minimum: 1 minute · 30 kills · 95% capture coverage. A complete normal route gives a more representative pace; duration alone does not guarantee accuracy.</div></section>
  <div class="calibration-side"><section class="card" id="quality-card">${typeof qualityContent==='function'?qualityContent(c?.quality):''}</section>${disclosure('calibration-help','Recording guide',`<ol class="instructions"><li><strong>Load your selected hero.</strong><p>Use your normal gear and talents. ForgePact is optional.</p></li><li><strong>Enter a regular Act region.</strong><p>Record your usual route. Walking and idle time count; town and loading do not.</p></li><li><strong>Finish and save.</strong><p>Finish before taking a break. Use the saved profile in Explore to start an expedition.</p></li></ol>`,'Before your first run')}</div></div>
  <section class="card" style="margin-top:22px"><h2>Saved profiles <span class="badge">${data.profiles.length} ${data.profiles.length===1?'record':'records'}</span></h2><div class="table-wrap" tabindex="0" role="region" aria-label="Saved calibration profiles"><table class="table"><thead><tr><th>Region / hero</th><th>Pace</th><th>Duration</th><th>Status</th></tr></thead><tbody>${data.profiles.map(p=>`<tr><td>${esc(zones.find(z=>z.room===p.room)?.name||p.room)}<small>${esc(p.character?.name||'Missing character identity')}</small></td><td>${num(p.kills_per_min,1)} / min</td><td>${num(p.basis_seconds/60,1)} min</td><td><span class="badge ${p.usable?'good':''}">${p.usable?(p.quality?.status==='steady'?'Steady sample':p.quality?.status==='variable'?'Variable pace':'Short sample'):'Recalibrate'}</span>${p.problems.length?`<small>${esc(p.problems[0])}</small>`:''}${p.usable?`<button class="inline-link profile-use" data-use-profile="${esc(p.id)}">Use profile</button>`:''}</td></tr>`).join('')||'<tr><td colspan="4">No profiles yet. Your first calibration will appear here.</td></tr>'}</tbody></table></div></section>`;
}
function loot(){
  const history=`<div class="card"><div class="label-row"><span>INFINITE VAULT</span><span class="badge ${data.editor?'good':''}">${data.editor?'Connected':'Item Editor offline'}</span></div>${!data.rewards.length?`<div class="empty">${svg('chest')}<h2>Your next haul belongs here.</h2><p>Complete an expedition and claim its rewards to see your items. They stay on this computer until you transfer them to the Vault.</p></div>`:`<div class="table-wrap" tabindex="0" role="region" aria-label="Reward delivery history"><table class="table"><thead><tr><th>Expedition</th><th>Loot</th><th>Game save</th><th>Vault</th></tr></thead><tbody>${data.rewards.map(r=>`<tr><td><strong>${esc(r.character?.name||'Hero')}</strong><small>${esc(roomName(r.room))}</small><small class="record-id" title="${esc(r.id)}">${esc(r.id)}</small></td><td>${num(r.items||0)} items<small>${num(r.gold||0)} gold</small></td><td><span class="badge ${r.save_confirmed?'good':''}">${r.save_confirmed?'Saved':r.partial?'Partial':'Needs review'}</span></td><td>${r.stages?.ingest==='done'?`<span class="badge good">Transferred</span><button class="inline-link" data-ingest="${esc(r.id)}" ${busy()||!data.editor?'disabled':''} title="Adds items your current filter allows that are not in the Vault yet">Transfer again</button>`:!r.save_confirmed&&!r.partial?`<a class="quiet" href="/api/recovery-report?id=${encodeURIComponent(r.id)}" target="_blank" rel="noopener">Review save receipt</a>`:`<button class="quiet" data-ingest="${esc(r.id)}" ${busy()||!data.editor?'disabled':''}>Transfer to Vault</button>`}</td></tr>`).join('')}</tbody></table></div>`}</div>`;
  return heading('REWARDS','Your loot, at a glance','Review your haul and transfer it to Infinite Vault.',data.editor?`<a class="quiet" href="${esc(data.editor)}" target="_blank" rel="noopener">${svg('link')} Open Vault</a>`:'')+jobBanner()+deliveryCard()+(data.rewards.length?disclosure('delivery-history','Delivery & Vault status',history,`${data.rewards.length} expedition${data.rewards.length===1?'':'s'}`,data.rewards.some(r=>r.stages?.ingest!=='done')):history);
}
function settings(){
  const install=data.installation;
  return heading('SETTINGS','Make yourself at home','Connect Hero Siege and personalize your local panel.')+jobBanner()+`<div class="content-grid"><section class="card"><span class="eyebrow">HERO SIEGE</span><div class="spacer"></div><h2>Game folder</h2><label class="muted" for="game-folder">The bin folder containing Hero_Siege.exe</label><input id="game-folder" class="settings-input" type="text" spellcheck="false" value="${esc(data.config.game_bin)}" placeholder="C:\\Games\\HeroSiege\\bin"><div class="actions"><button class="primary" data-action="configure">Save folder</button><button class="quiet" data-action="install" ${busy()||!install.configured?'disabled':''}>Install AFK plugin</button></div><p class="settings-note">Close the game before installing the plugin. Your existing AFK DLL is backed up automatically. Aurie and YYToolkit must already be installed.</p><div class="check-row"><span>Game folder</span><strong class="${install.exe?'':'missing'}">${install.exe?'Found':'Not selected'}</strong></div><div class="check-row"><span>Aurie / YYToolkit</span><strong class="${install.aurie&&install.yytk?'':'missing'}">${install.aurie&&install.yytk?'Found':'Needs attention'}</strong></div><div class="check-row"><span>AFK plugin</span><strong class="${install.plugin?'':'missing'}">${install.plugin?'File found':'Not installed'}</strong></div><div class="check-row"><span>Live connection</span><strong class="${data.live?'':'missing'}">${data.live?esc(data.live.plugin||'Connected'):'Game disconnected'}</strong></div></section><section class="card"><span class="eyebrow">LOCAL TOOLS</span><h2 class="section-title">On your computer</h2><p class="muted">No account or internet needed. Your expedition timer keeps running when this panel is closed.</p><div class="actions"><button class="quiet" data-action="log">${svg('info')} Activity log</button><button class="quiet" data-action="close" ${busy()||!data.game_running?'disabled':''}>Close game</button></div><p class="settings-note">The activity log keeps original command output for troubleshooting.</p></section></div>`;
}
function render(){
  if(!data)return;
  const mapFocus=document.activeElement;
  const controlFocus=mapFocus?.id&&/^[\w-]+$/.test(mapFocus.id)?'#'+mapFocus.id:null;
  for(const el of document.querySelectorAll?.('details[data-disclosure]')||[])disclosureState.set(el.dataset.disclosure,el.open);
  ...
  const c=current();
  document.querySelector('#app').innerHTML=`<div class="shell"><aside class="rail"><button class="brand" data-view="explore" aria-label="AFK FARM home"><img src="/assets/emblem.svg" alt=""><span>AFK <b>FARM</b><small>HERO SIEGE</small></span></button><nav class="nav" aria-label="Main navigation">${[['explore','map','Explore'],['measure','timer','Calibration'],['loot','chest','Loot'],['modifiers','sliders','Modifiers'],['settings','settings','Settings']].map(([v,i,label])=>`<button class="nav-button ${view===v?'active':''}" data-view="${v}" ${view===v?'aria-current="page"':''}>${svg(i)}<span>${label}</span></button>`).join('')}</nav><div class="rail-bottom"><span class="live-dot"></span> Your world. Your pace.</div></aside><main><header class="topbar"><div class="wordmark"><span class="topbar-section">${({explore:'Expeditions',measure:'Calibration',loot:'Loot chest',modifiers:'Reward modifiers',settings:'Settings'})[view]}</span><small>OFFLINE COMPANION</small></div><div class="top-actions"><span class="status-pill ${data.live||deliveryActive()?'':'off'}"><i class="live-dot"></i>${esc(gameStatus())}</span><div class="character-picker"><div class="avatar" aria-hidden="true">${svg('swords')}</div><div><label for="character">SELECTED HERO</label><select id="character">${data.characters.map(ch=>`<option value="${ch.slot}" ${ch.slot===selected?'selected':''}>${esc(ch.name)} · ${esc(ch.class_name)} · Lv. ${ch.level} · Slot ${ch.slot+1}</option>`).join('')||'<option>No local characters found</option>'}</select></div></div></div></header><div class="workspace" id="main-content" tabindex="-1">${view==='explore'?explore():view==='measure'?measurement():view==='loot'?loot():view==='modifiers'?modifiers():settings()}<footer class="footer"><span>${svg('check')} Local &amp; offline · v${esc(data.version)}</span><span><button data-action="log">Activity log ↗</button></span></footer></div></main></div>`;
  extrasPanel();bindMap();updateLive();
  if(mapFocusSelector||controlFocus)document.querySelector(mapFocusSelector||controlFocus)?.focus?.({preventScroll:true});
}
function updateLive(){
  ...
  if(data.armed){
    ...
    const note=document.getElementById('expedition-note');if(note)note.textContent=deliveryNeedsReview()?'Partial delivery is on hold. Review the save and reward records before any further delivery.':deliveryActive()?deliveryTitle()+'. '+deliveryMessage():data.progress.resumable?(data.progress.resume_accepted?'Delivery stopped at a recorded position you accepted. Continue with the same hero in the same region.':'Delivery paused at a saved position. Continue with the same hero in the same region.'):remaining?'remaining · '+num(credit/3600,2)+' hours accrued':'Expedition time is complete. Return to the recorded hero and region to generate rewards.';
...
document.addEventListener('input',event=>{
  ...
  document.querySelectorAll('[data-hours]').forEach(b=>{const active=Number(b.dataset.hours)===hours;b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active));});
});
...
// All deferred UI modules must be ready before the first asynchronous render.
document.addEventListener('DOMContentLoaded',async()=>{try{zones=await(await fetch('/zones.json')).json();world=zone().world;await poll();centerMap();}catch(error){toast(error.message);}},{once:true});
